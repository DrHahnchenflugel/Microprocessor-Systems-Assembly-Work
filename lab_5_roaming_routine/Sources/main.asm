...
DISPATCHER      CMPA  #START                    ; If it’s the START state ------------------+
                ...
                BRA   DISP_EXIT                 ; and exit                                  I
                                                ;                                           S
NOT_START       CMPA  #FWD                      ; Else if it’s the FWD state                P
                BNE   NOT_FORWARD               ;                                           A
                JSR   FWD_ST                    ; then call the FWD routine                 T
                JMP   DISP_EXIT                 ; and exit                                  C
                                                ;                                           H
NOT_FORWARD     CMPA  #REV                      ; Else if it’s the REV state                E
                BNE   NOT_REVERSE               ;                                           R
                JSR   REV_ST                    ; then call the REV routine                 |
                JMP   DISP_EXIT                 ; and exit                                  |
                                                ;                                           |
NOT_REVERSE     CMPA  #ALL_STP                  ; Else if it’s the ALL_STP state            |
                BNE   NOT_ALL_STOP              ;                                           |
                JSR   ALL_STP_ST                ; then call the ALL_STP routine             |
                JMP   DISP_EXIT                 ; and exit                                  |
                                                ;                                           |
NOT_ALL_STOP    CMPA  #FWD_TRN                  ; Else if it’s the FWD_TURN state           |
                BNE   NOT_FWD_TRN               ;                                           |
                JSR   FWD_TRN_ST                ; then call the FWD_TURN routine            |
                JMP   DISP_EXIT                 ; and exit                                  |
                                                ;                                           |
NOT_FWD_TRN     CMPA  #REV_TRN                  ; Else if it’s the REV_TRN state            |
                ...
                BRA   DISP_EXIT                 ; and exit                                  |
                                                ;                                           |
NOT_REV_TRN     SWI                             ; Else CRNT_ST is not defined - ERROR! exit.|
...
NO_FWD_BUMP     BRSET PORTAD0,$08,NO_REAR_BUMP  ; If REAR_BUMP
                JSR   INIT_ALL_STP              ; initialize ALL_STP
                MOVB  #ALL_STP,CRNT_STATE       ; and change CRNT_STATE to ALL_STP
                JMP   FWD_EXIT                  ; 

NO_REAR_BUMP    LDAA  TOF_COUNTER               ; If Tc>Tfwd
...
REV_TRN_ST      LDAA  TOF_COUNTER               ; If Tc>Trevturn then
                ...
                BRA   REV_TRN_EXIT              ; 
NO_FWD_RT       NOP                             ; else
REV_TRN_EXIT    RTS                             ; return

;***********************************************
INIT STATE SUBROUTINES
;***********************************************
...
INIT_REV        BSET  PORTA,%00000011           ; Set rev direction for both motors
                BSET  PTT,%00110000             ; Turn on the drive motors
                LDAA  TOF_COUNTER               ; Mark the rev time T_rev
                ADDA  #REV_INT
                STAA  T_REV
                RTS
...
;***********************************************
INIT_FWD_TRN    BSET  PORTA,%00000010           ; Set reverse direction on starboard motor
                LDAA  TOF_COUNTER               ; Mark the fwd_turn time T_fwdturn
                ADDA  #FWD_TRN_INT
                STAA  T_FWD_TRN
                RTS
                
;***********************************************
INIT_REV_TRN    BCLR  PORTA,%00000010           ; Set forward dir. for starboard motor
                LDAA  TOF_COUNTER               ; Mark the fwd time T_fwd
                ADDA  #REV_TRN_INT
                STAA  T_REV_TRN
                RTS
...
;***********************************************
del_50us:       PSHX                            ;2 E-clk X reg is protected, as we use x in this loop
eloop:          LDX   #300                      ;2 E-clk
iloop:          NOP                             ;1 E-clk
                DBNE  X,iloop                   ;3 E-clk
                DBNE  Y,eloop                   ;3 E-clk
                PULX                            ;3 E-clk X reg content is returned
                RTS                             ;5 E-clk
                
;***********************************************
cmd2LCD:        BCLR  LCD_CNTR,LCD_RS           ; select Instruction Register
                JSR   dataMov                   ; send data
      	        RTS
      	        
;***********************************************
putsLCD         LDAA  1,X+                      ; get one character from the string
                BEQ   donePS                    ; if null (end), break to end
                JSR   putcLCD                   ; else, put char on LCD 
                BRA   putsLCD                   ; and move to next
donePS 	        RTS

;***********************************************
putcLCD         BSET  LCD_CNTR,LCD_RS           ; select Data register
                JSR   dataMov                   ; send data
                RTS
...
BCD2ASC         LDAA  #$0                       ; Initialize the blanking flag
    ...
    NOT_BLANK1: LDAA  TEN_THOUS                 ; Get the ’ten_thousands’ digit
      ...
      ISBLANK2: LDAA  #$20                      ; Thousands digit is blank
