...
; LDR Threshold Equates
; Threshold above which white is detected, below which is black
;------------------------------
FORE_PATTERN_THRESH   EQU   $47     ; Sensor 'A' (Line, Fore)
MID_PATTERN_THRESH    EQU   $69     ; Sensor 'C' (Line, Mid)
PORT_PATTERN_THRESH   EQU   $6A     ; Sensor 'D' (Edge, Port)
STAR_PATTERN_THRESH   EQU   $59     ; Sensor 'B' (Edge, Starboard)
DIFF_LINE_THRESH      EQU   $74     ; Differential Sens Pair E-F. Higher val = too far left

; TURN INTERVAL VALUES
LEFT_TURN_INTERVAL     EQU   46                        ; 2 second delay (at 23Hz)
RIGHT_TURN_INTERVAL    EQU   46                        ; 2 second delay (at 23Hz)
SPEED_CONTROL_ON_INTERVAL EQU   1      ; 3/23 sec delay
SPEED_CONTROL_OFF_INTERVAL EQU  1     ; 2 sec delay

; STATE VALUES
STATE_LF_LEFT                EQU   0
STATE_LF_RIGHT               EQU   1 
STATE_LF_LEFT_STOP           EQU   2
STATE_LF_RIGHT_STOP          EQU   3
STATE_LEFT_TURN              EQU   4
STATE_RIGHT_TURN             EQU   5
STATE_T_LEFT_RIGHT           EQU   6
STATE_T_FWD_LEFT             EQU   7
STATE_T_FWD_RIGHT            EQU   8
STATE_L_LEFT                 EQU   9
STATE_L_RIGHT                EQU   10
STATE_DEAD                   EQU   11
...
TOP_LINE      FCC   "                    " ;(20) Top line of display
              FCB   NULL      ; terminated by null
BOT_LINE      FCC   "  |  |  |  |  |     " ;(20) Bottom line of display
              FCB   NULL      ; terminated by null
CLEAR_LINE    FCC   ' '
              FCB   NULL      ; terminated by null
...
; TODO - work with large times, SPEED_CONTROL etc will max out but TOF_COUNTER wont.
T_RIGHT_TURN    ds.b  10                         ; RIGHT time
T_LEFT_TURN     ds.b  10                         ; LEFT time
T_SPEED_CONTROL ds.b  10                         ; SPEED CONTROL time
...
INIT 
              ...
              BSET  DDRT,%00110000 ; MAKE PORTT an output (STAR_SPEED, PORT_SPEED)
              RTS
...
MAIN_DISPATCHER
                  ...
                  BLT  CENTER_WHITE
                  ; CENTER BLACK - ON LINE OR JUNCTION
                  
                  LDAA SENSOR_PORT
                  CMPA PORT_PATTERN_THRESH
                  BLT  PORT_WHITE
                  ; CENTER BLACK - JUNCTION OF SOME SORT
                  
                  LDAA SENSOR_BOW
                  CMPA FORE_PATTERN_THRESH
                  BLT PORT_B_FORE_W
                  ;CENTER B, PORT B, BOW B - T junction l+f
                  MOVB #STATE_T_FWD_LEFT,CRNT_STATE        
                  BRA END_MAIN_DISPATCHER

PORT_B_FORE_W
                  LDAA SENSOR_STBD
                  CMPA STAR_PATTERN_THRESH
                  BLT PORT_B_FORE_W_STBD_W
                  ;T junction                      
                  MOVB #STATE_T_LEFT_RIGHT,CRNT_STATE
                  BRA END_MAIN_DISPATCHER

PORT_B_FORE_W_STBD_W
                  ; right angle leftwards? 
                  MOVB #STATE_L_LEFT,CRNT_STATE
                  BRA END_MAIN_DISPATCHER
...
PORT_WHITE
                  ...
                  CMPA STAR_PATTERN_THRESH
                  BLT  STAR_WHITE
                  ; CENTER B LEFT W RIGHT B - check more junctions
                  
                  LDAA SENSOR_BOW
                  CMPA FORE_PATTERN_THRESH
                  BLT STBD_B_FORE_W    
                  MOVB #STATE_T_FWD_RIGHT,CRNT_STATE
                  ; T junction right+fwd 
                  BRA END_MAIN_DISPATCHER

STBD_B_FORE_W
                  ;right angle rightwards?
                  BRA END_MAIN_DISPATCHER
...
CENTER_WHITE      ; CENTER NOT WHITE - OFF LINE!  
                  LDAA SENSOR_PORT
                  CMPA PORT_PATTERN_THRESH
                  BLT  CENTER_W_PORT_W       
                  MOVB #STATE_LEFT_TURN,CRNT_STATE
                  ; CENTER WHITE LEFT BLACK - TURN LEFT 
                  JSR STARFWD
                  JSR PORTOFF
                  JSR STARON
...
CENTER_W_PORT_W
                  LDAA SENSOR_STBD
                  CMPA STAR_PATTERN_THRESH
                  BLT CENTER_W_PORT_W_STBD_W 
                  MOVB #STATE_RIGHT_TURN,CRNT_STATE
                  ; CENTER WHITE LEFT W RIGHT BLACK - TURN RIGHT
                  JSR PORTFWD
                  JSR PORTON
                  JSR STAROFF
                  BRA END_MAIN_DISPATCHER

CENTER_W_PORT_W_STBD_W
                  LDAA SENSOR_BOW
                  CMPA FORE_PATTERN_THRESH
                  BLT DEAD
                  ; CENTER W LEFT W RIGHT W FORE B - DEAD
                  BRA DEAD

DEAD              ; ALL WHITE - DEAD 
                  MOVB #STATE_DEAD,CRNT_STATE
END_MAIN_DISPATCHER
                  RTS                                  
;***********************************************
LINE_FOLLOW_DISPATCHER      
                  CMPA  #STATE_LF_LEFT                    
                  BNE   NOT_LF_LEFT     
                  JSR   LF_LEFT                  
                  BRA   DISP_EXIT                
...
NOT_LF_RIGHT    
                  CMPA  #STATE_LF_LEFT_STOP           
                  BNE   NOT_LF_LEFT_STOP         
                  JSR   LF_LEFT_STOP            
                  BRA   DISP_EXIT                 
                                              
NOT_LF_LEFT_STOP     
                  CMPA  #STATE_LF_RIGHT_STOP                
                  BNE   NOT_LF_RIGHT_STOP              
                  JSR   LF_RIGHT_STOP             
                  BRA   DISP_EXIT                
...
;---------------------------------------------------------------------------
; Follow the line
; This routine takes the value of the middle line differential pair of 
; sensors, compares it to the threshold, and turns right or left from there
; The speed is controlled to a 50% duty cycle
;
; This routine will only be called when the robot detects that it's on a 
; valid line.
;---------------------------------------------------------------------------              
;****************LINE FOLLOW LEFT TURN************************
INIT_LF_LEFT               
              JSR   STARFWD
              JSR   STARON
              JSR   PORTOFF
              
              LDAA  TOF_COUNTER               ; Mark the fwd time T_fwd
              ADDA  #SPEED_CONTROL_ON_INTERVAL
              STAA  T_SPEED_CONTROL
              RTS

LF_LEFT
              LDAA SENSOR_LINE                              ; Load values of middle diff pair
              CMPA DIFF_LINE_THRESH;
              
              BLT  LF_LEFT_CHK_TIMER            
              JSR INIT_LF_RIGHT
              MOVB  #STATE_LF_RIGHT,CRNT_STATE 
                                                    ; UNTIMED values are used, we don't know how far to turn
              JSR LF_LEFT_DONE

LF_LEFT_CHK_TIMER      
              LDAA  TOF_COUNTER               ; If Tc>Trev then
              CMPA  T_SPEED_CONTROL                     ; the robot should make a FWD turn
              BLT   LF_LEFT_DONE
              JSR   INIT_LF_LEFT_STOP
              MOVB  #STATE_LF_LEFT_STOP,CRNT_STATE                                       ; UNTIMED values are used, we don't know how far to turn
              JSR LF_LEFT_DONE
              
LF_LEFT_DONE
              RTS

;****************LINE FOLLOW RIGHT TURN************************
INIT_LF_RIGHT    
              JSR   PORTFWD
              JSR   STAROFF
              JSR   PORTON           
...
LF_RIGHT
              ...
              LDAA SENSOR_LINE                              ; Load values of middle diff pair
              CMPA DIFF_LINE_THRESH;
              BGT  LF_RIGHT_CHK_TIMER
              JSR INIT_LF_LEFT
              MOVB  #STATE_LF_LEFT,CRNT_STATE                                       ; UNTIMED values are used, we don't know how far to turn
              JSR LF_RIGHT_DONE
...
LF_RIGHT_CHK_TIMER    
              ...
              JSR   INIT_LF_RIGHT_STOP
              MOVB  #STATE_LF_RIGHT_STOP,CRNT_STATE                                       ; UNTIMED values are used, we don't know how far to turn
              JSR LF_RIGHT_DONE
              
LF_RIGHT_DONE
              RTS
;****************LINE FOLLOW LEFT TURN STOP********************
INIT_LF_LEFT_STOP          
              JSR   STAROFF
              JSR   PORTOFF
              LDAA  TOF_COUNTER               ; Mark the fwd time T_fwd
              ADDA  #SPEED_CONTROL_OFF_INTERVAL
              STAA  T_SPEED_CONTROL
              RTS
...
LF_LEFT_STOP
              ...
              JSR   INIT_LF_LEFT
              MOVB  #STATE_LF_LEFT,CRNT_STATE                                       ; UNTIMED values are used, we don't know how far to turn
              JSR LF_LEFT_STOP_DONE
              
LF_LEFT_STOP_DONE
              RTS

;****************LINE FOLLOW RIGHT TURN STOP********************
INIT_LF_RIGHT_STOP 
...
LF_RIGHT_STOP
              ...
              BLT   LF_RIGHT_DONE
              JSR   INIT_LF_RIGHT
              MOVB  #STATE_LF_RIGHT,CRNT_STATE                                       ; UNTIMED values are used, we don't know how far to turn
              JSR LF_RIGHT_STOP_DONE
...
STRCPY 
              PSHX ; Protect the registers used
              PSHY
              PSHA
STRCPY_LOOP 
...
SELECT_SENSOR 
                PSHA ; Save the sensor number for the moment
...
STATE_POSITION    EQU   TOP_LINE+13
