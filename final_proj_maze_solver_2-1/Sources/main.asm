...
;------------------------------
FORE_PATTERN_THRESH   EQU   $FF     ; Sensor 'A' (Line, Fore)
AFT_PATTERN_THRESH    EQU   $FF     ; Sensor 'C' (Line, Aft)
PORT_PATTERN_THRESH   EQU   $FF     ; Sensor 'D' (Edge, Port)
STAR_PATTERN_THRESH   EQU   $FF     ; Sensor 'B' (Edge, Starboard)
DIFF_LINE_THRESH      EQU   $99     ; Differential Sens Pair E-F. Higher val = too far left

; TURN INTERVAL VALUES
LEFT_TURN_INTERVAL     EQU   46                        ; 2 second delay (at 23Hz)
RIGHT_TURN_INTERVAL    EQU   46                        ; 2 second delay (at 23Hz)
SPEED_CONTROL_INTERVAL EQU   10      ; 10/23 sec delay
...
T_RIGHT_TURN    ds.b  1                         ; RIGHT time
T_LEFT_TURN     ds.b  1                         ; LEFT time
T_SPEED_CONTROL ds.b  1                         ; SPEED CONTROL time
...
DISPATCHER      
                ...
                JSR   LF_LEFT                  ; then call START_ST routine                D
                BRA   DISP_EXIT                 ; and exit                                  I
                                                ;                                           S
NOT_LF_LEFT       CMPA  #STATE_LF_RIGHT                    ; Else if itâ€™s the FWD state                P
                BNE   NOT_LF_RIGHT               ;                                           A
                
                JSR   LF_RIGHT                   ; then call the FWD routine                 T
                JMP   DISP_EXIT                 ; and exit                                  C
                                                ;                                           H
NOT_LF_RIGHT     CMPA  #STATE_LF_LEFT_STOP                      ; Else if itâ€™s the REV state                E
                BNE   NOT_LF_LEFT_STOP               ;                                           R
                JSR   LF_LEFT_STOP                  ; then call the REV routine                 |
                JMP   DISP_EXIT                 ; and exit                                  |
                                                ;                                           |
NOT_LF_LEFT_STOP     CMPA  #STATE_LF_RIGHT_STOP                 ; Else if itâ€™s the ALL_STP state            |
                BNE   NOT_LF_RIGHT_STOP              ;                                           |
                JSR   LF_RIGHT_STOP              ; then call the ALL_STP routine             |
                JMP   NOT_LF_RIGHT_STOP                 ; and exit                                  |

NOT_LF_RIGHT_STOP     SWI                             ; Else CRNT_ST is not defined - ERROR! exit.|
